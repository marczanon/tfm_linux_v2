...
from codigo.app.executors.cleaning import DEFAULT_CLEANING_CONFIG
...
DEFAULT_CLEAN_OUTPUT_PATH = "codigo/data/processed/cwru_bearing/clean_signals"
...
def decide_cleaning_action_deterministic(state: TFMStateModel) -> CleaningDecision:
    ...
    return CleaningDecision(
        ...
        rationale=(
            "Fallback CWRU cleaning policy: remove non-finite values, resample to "
            "the target sample rate and avoid normalization before baseline modeling."
        ),
        confidence=1.0,
        cleaning_config=DEFAULT_CLEANING_CONFIG,
        expected_artifact_path=DEFAULT_CLEAN_OUTPUT_PATH,
        warnings=[],
    )
        ...
        LLMMessage(
            ...
            content="\n".join(
                [
                    ...
                    "- expected_artifact_path debe apuntar al directorio limpio CWRU.",
